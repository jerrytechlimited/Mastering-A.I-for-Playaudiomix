...
// Enhanced exciter function using waveshaping
function createExciterWaveShaper(audioCtx, amount) {
  ...
  for (let i = 0; i < samples; i++) {
    ...
    const drive = 1 + (amount * 4); // Scale exciter amount
    let y = x * drive;
    
    // Apply soft clipping with harmonic generation
    if (Math.abs(y) > 0.7) {
      y = Math.sign(y) * (0.7 + (Math.abs(y) - 0.7) / (1 + Math.abs(y) - 0.7));
    }
    
    // Add subtle harmonic enhancement
    y += Math.sin(x * Math.PI * 2) * amount * 0.1;
...
async function applyMastering2(targetBuffer, referenceFeatures, userParams = {}) {
  ...
  // Create exciter effect
  let exciterNode = null;
  let exciterHighpass = null;
  let exciterGain = null;
  if (userParams.exciter && userParams.exciter > 0) {
    // High-pass filter to isolate high frequencies for exciter
    exciterHighpass = audioCtx.createBiquadFilter();
    exciterHighpass.type = 'highpass';
    exciterHighpass.frequency.value = 3000; // Apply exciter to frequencies above 3kHz
    exciterHighpass.Q.value = 0.7;
    
    // Exciter waveshaper
    exciterNode = createExciterWaveShaper(audioCtx, userParams.exciter);
    
    // Gain control for exciter mix
    exciterGain = audioCtx.createGain();
    exciterGain.gain.value = userParams.exciter * 0.3; // Control exciter intensity
  }
  ...
  // Create a mixer node for combining original and excited signals
  let mixerNode = audioCtx.createGain();
  ...
  // Connect main signal path through EQ and compressor
  lastNode.connect(eqLow).connect(eqMid).connect(eqHigh).connect(comp);
  
  // Connect main signal to mixer
  comp.connect(mixerNode);
  
  // If exciter is enabled, create parallel processing path
  if (exciterNode && exciterHighpass && exciterGain) {
    // Send signal to exciter path
    comp.connect(exciterHighpass);
    exciterHighpass.connect(exciterNode);
    exciterNode.connect(exciterGain);
    // Mix excited signal back with main signal
    exciterGain.connect(mixerNode);
  }

  if (stereoNode) {
    mixerNode.connect(stereoNode.splitter);
...
let fxCtx2 = null, fxSource2 = null, fxGain2 = null, fxNoise2 = null, fxExciterHighpass2 = null, fxExciterNode2 = null, fxExciterGain2 = null, fxMixer2 = null, fxSplitter2 = null, fxLeftGain2 = null, fxRightGain2 = null, fxMerger2 = null;
...
function clearFX2() {
  ...
  fxCtx2 = fxSource2 = fxGain2 = fxNoise2 = fxExciterHighpass2 = fxExciterNode2 = fxExciterGain2 = fxMixer2 = fxSplitter2 = fxLeftGain2 = fxRightGain2 = fxMerger2 = null;
  fxIsPlaying2 = false;
  fxStartTime2 = 0;
  fxDuration2 = 0;
...
function makeRealtimeFXChain(buffer, params) {
  ...
  // Create mixer for combining original and excited signals
  fxMixer2 = fxCtx2.createGain();
  ...
  // Create exciter chain
  if (params.exciter && params.exciter > 0) {
    fxExciterHighpass2 = fxCtx2.createBiquadFilter();
    fxExciterHighpass2.type = 'highpass';
    fxExciterHighpass2.frequency.value = 3000;
    fxExciterHighpass2.Q.value = 0.7;
    
    fxExciterNode2 = createExciterWaveShaper(fxCtx2, params.exciter);
    
    fxExciterGain2 = fxCtx2.createGain();
    fxExciterGain2.gain.value = params.exciter * 0.3;
  }
  ...
  // Connect main signal to mixer
  last.connect(fxMixer2);
  
  // Connect exciter path if enabled
  if (fxExciterHighpass2 && fxExciterNode2 && fxExciterGain2) {
    last.connect(fxExciterHighpass2);
    fxExciterHighpass2.connect(fxExciterNode2);
    fxExciterNode2.connect(fxExciterGain2);
    fxExciterGain2.connect(fxMixer2);
  }
...
document.getElementById('processBtn2').onclick = async () => {
  ...
  try {
    ...
    let userParams = { gain: 1, noiseReduction: 0, stereoWidth: 1, exciter: 0 };
...
// Updated control listeners to include exciter
  ...
  payModal.innerHTML = `
    <div style="background:#fff;padding:2em;max-width:400px;margin:10vh auto;border-radius:8px;box-shadow:0 4px 24px #222;">
      ...
      <div style="margin:2em 0;display:flex;justify-content:space-between;">
       <a> <span id="paystackBtn2" style="flex:1;margin-right:1em;">
        ...
        Paystack</span>
        <div id="paypalBtn2" style="flex:1;"></div></a>
      </div>
  ...
  paypal.Buttons({
    ...
    createOrder: function(data, actions) {
      return actions.order.create({
        purchase_units: [{
          amount: { value: '4' } // $4.00 USD, change as needed!
        }]
      });
    },
...
-------------------------
<!-- Target Audio Upload -->
<input type="file" id="targetAudio2" accept="audio/*">
...
<!-- Process Button -->
<button id="processBtn2">Master Audio</button>

<!-- Progress Bar -->
<div id="progressBar2" style="width: 0%; height: 20px; background: #007bff;"></div>
<span id="progressText2">0%</span>

<!-- Status -->
<div id="status2">Ready to process...</div>

<!-- Target Player Section -->
<div id="targetPlayerSection2" style="display: none;">
  <h3>Original Audio</h3>
  <button id="targetPlayPause2" disabled>▶</button>
  <div id="targetProgressBar2" style="width: 100%; height: 10px; background: #eee; cursor: pointer;">
    <div id="targetProgressFill2" style="width: 0%; height: 100%; background: #007bff;"></div>
  </div>
  <span id="targetTime2">0:00 / 0:00</span>
</div>

<!-- Output Player Section -->
<div id="outputPlayerSection2" style="display: none;">
  <h3>Mastered Audio Preview</h3>
  <button id="fxPlayPause2" disabled>▶</button>
  <div id="fxProgressBar2" style="width: 100%; height: 10px; background: #eee; cursor: pointer;">
    <div id="fxProgressFill2" style="width: 0%; height: 100%; background: #28a745;"></div>
  </div>
  <span id="fxTime2">0:00 / 0:00</span>
</div>

<!-- Controls Section -->
<div id="controlsSection2" style="display: none;">
  <h3>Fine-tune Controls</h3>
  
  <!-- Gain Control -->
  <label>Gain: <span id="gainVal2">1</span></label>
  <input type="range" id="gainControl2" min="0.1" max="2" step="0.1" value="1">
  
  <!-- Noise Reduction -->
  <label>Noise Reduction: <span id="noiseVal2">0</span></label>
  <input type="range" id="noiseReduction2" min="0" max="1" step="0.1" value="0">
  
  <!-- Stereo Width -->
  <label>Stereo Width: <span id="widthVal2">1</span></label>
  <input type="range" id="stereoWidth2" min="0" max="2" step="0.1" value="1">
  
  <!-- Exciter Control (NEW) -->
  <label>Exciter: <span id="exciterVal2">0</span></label>
  <input type="range" id="exciterControl2" min="0" max="1" step="0.05" value="0">
</div>

<!-- Download Link -->
<a href="#" id="downloadLink2" style="display: none;">Download Mastered Track</a>
-------------------------
*/
