...
async function applyMastering2(targetBuffer, referenceFeatures, userParams = {}) {
  ...
  // Split into two parallel paths: main and exciter
  const mainChain = audioCtx.createGain();
  lastNode.connect(mainChain);

  const exciterChain = audioCtx.createGain();
  lastNode.connect(exciterChain);

  mainChain.connect(eqLow).connect(eqMid).connect(eqHigh);

  exciterChain.connect(exciterHPF);
  exciterHPF.connect(exciterNode).connect(exciterGain);

  const mainMerger = audioCtx.createChannelMerger(2);
  eqHigh.connect(mainMerger, 0, 0);
  eqHigh.connect(mainMerger, 0, 1);
